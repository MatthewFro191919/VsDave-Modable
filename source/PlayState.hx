...
class PlayState extends MusicBeatState {
	...
	public static var funnyFloatyBoys:Array<String> = [
		'dave-angey',
		'bambi-3d',
		'expunged',
		'bambi-unfair',
	...
	override public function create() {
		...
		shaggyVoice = isShaggy && [
			'warmup', 'house', 'insanity', 'polygonized', 'blocked', 'corn-theft', 'maze', 'splitathon', 'shredder', 'greetings', 'interdimensional', 'rano',
			'bonus-song', 'bot-trot', 'escape-from-california', 'adventure', 'mealie', 'indignancy', 'memory', 'roofs', 'supernovae', 'glitch', 'master',
			'cheating', 'unfairness', 'kabunga', 'recursed', 'exploitation'
		...
		healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, inFiveNights || oppM ? LEFT_TO_RIGHT : RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8),
			Std.int(healthBarBG.height - 8), this, 'curHealth', 0, 2);
			...
			case 'exploitation':
				if (!modchartoption)
					...
					credits = LanguageManager.getTextString('exploitation_credit')
						+ " "
						+ (!FlxG.save.data.selfAwareness ? CoolSystemStuff.getUsername() : (shaggyVoice ? 'Shaggy' : 'Boyfriend'))
						+ "!";
			case 'kabunga':
	...
	public function createBackgroundSprites(bgName:String, revertedBG:Bool):FlxTypedGroup<BGSprite> {
		...
		switch (bgName) {
			...
			case 'desert':
				...
				desertbg1 = new BGSprite('desert2', desertBG.x - desertBG.width, desertBG.y,
					Paths.image('backgrounds/wedcape_from_cali_backlground', 'shared'), null, 1, 1, true);
				desertbg1.setGraphicSize(Std.int(desertbg1.width * 1.2));
				desertbg1.updateHitbox();
				sprites.add(desertbg1);
				add(desertbg1);
			...
			default:
				if (FileSystem.exists(TitleState.modFolder + '/data/stages/' + bgName + '.json') /*&& FreeplayState.isaCustomSong*/) {
					...
					for (i in jsonStage.backgrounds) {
						createCustomBackground(i.spriteName, i.posX, i.posY, i.image, i.size, i.scrollX, i.scrollY, i.antialiasing, bgName, i.voidShader,
							i.updateHitBox, i.alpha, i.flying, i.animated, i.animation, i.showsWhenLowQuality, i.shade);
					}
				} else {
					bgZoom = 0.9;
					stageName = 'stage';
	...
	private function createCustomBackground(spriteName:String = 'test', posX:Float = 1, posY:Float = 1, imageName:String = 'none', size:Float = 1,
			scrollX:Float = 1, scrollY:Float = 1, antialiasings:Bool = true, bgName:String, isVoid:Bool = false, updateHitBox:Bool = true, alphaa:Float = 1,
			flying:Bool = false, animated:Bool = false, animation:String = 'idle', lQ:Bool = false, shade:String = ''):Void {
		...
		if (flying) {
			...
			var customBG = new BGSprite(spriteName, posX, posY, TitleState.modFolder + '/images/stages/' + bgName + '/' + imageName,
				[new Animation(animation, animation, 24, true, [false, false])], scrollX, scrollY, antialiasings, true, true);
			...
			var customBG:BGSprite = new BGSprite(spriteName, posX, posY, TitleState.modFolder + '/images/stages/' + bgName + '/' + imageName, null, scrollX,
				scrollY, antialiasings, false, true);
			customBG.setGraphicSize(Std.int(customBG.width * size));
			if (updateHitBox) {
				// trace('yeah');
				customBG.updateHitbox();
	...
	private function generateSong(dataPath:String):Void {
		...
		if (SONG.needsVoices
			&& FileSystem.exists(Paths.voicesPath(PlayState.SONG.song,
		...
		if (SONG.needsVoices
			&& FileSystem.exists(Paths.voicesPath(PlayState.SONG.song,
				localFunny == CharacterFunnyEffect.Tristan ? "-Tristan" : lucarioVoice ? "Lucario" : ""))) {
			vocals = new FlxSound().loadEmbedded(Paths.voices(PlayState.SONG.song,
				localFunny == CharacterFunnyEffect.Tristan ? "-Tristan" : lucarioVoice ? "Lucario" : ""));
		} else {
			vocals = new FlxSound();
	...
	private function generateStaticArrows(player:Int, regenerate:Bool = false, fadeIn:Bool = true):Void {
		...
		for (i in 0...Main.keyAmmo[mania]) {
			...
			if (((funnyFloatyBoys.contains(dad.curCharacter) || threedBoysMod.contains(dad.curCharacter)) && player == playername0)
				|| ((funnyFloatyBoys.contains(boyfriend.curCharacter) || threedBoysMod.contains(boyfriend.curCharacter))
					&& player == playername1)) {
				strumType = '3D';
			} else {
				switch (curStage) {
	...
	override public function update(elapsed:Float) {
		...
		if (SONG.song.toLowerCase() == 'escape-from-california') {
			...
			if (desertBG != null) {
				...
				desertbg1.x = desertBG.x - desertBG.width;
				desertbg1.y = desertBG.y;
			}
		...
		if (funnyFloatyBoys.contains(dad.curCharacter.toLowerCase())
			|| floatyBoysMod.contains(dad.curCharacter.toLowerCase())
			&& canFloat) {
		...
		if (funnyFloatyBoys.contains(boyfriend.curCharacter.toLowerCase())
			|| floatyBoysMod.contains(boyfriend.curCharacter.toLowerCase())
			&& canFloat) {
		...
		switch (SONG.song.toLowerCase()) {
			...
			case 'exploitation':
				scoreTxt.text = "Scor3: "
					+ (songScore * (modchartoption ? FlxG.random.int(1, 9) : 1))
					+ " | M1ss3s: "
					+ (misses * (modchartoption ? FlxG.random.int(1, 9) : 1))
					+ " | Accuracy: "
					+ (truncateFloat(accuracy, 2) * (modchartoption ? FlxG.random.int(1, 9) : 1))
					+ "% ";
			default:
				if (SONG.exploitationEffect) {
					scoreTxt.text = "Scor3: "
	...
	private function popUpScoreSus(note:Note):Void {
		...
		scoreS = cast(FlxMath.roundDecimal(cast(scoreS, Float) * curmultDefine[note.noteData],
			0), Int); // this is old code thats stupid Std.Int exists but i dont feel like changing this
			...
			switch (note.noteStyle) {
				...
				case 'phone-alt':
					if (oppM) {
						...
						var noteTypes = guitarSection ? notestuffsGuitar : notestuffs;
						fuckingDumbassBullshitFuckYou = noteTypes[Math.round(Math.abs(note.originalType)) % playerStrumAmount];
						if (noteTypes[Math.round(Math.abs(note.originalType)) % playerStrumAmount] == 'UP')
							fuckingDumbassBullshitFuckYou = 'RIGHT';
						if (noteTypes[Math.round(Math.abs(note.originalType)) % playerStrumAmount] == 'DOWN')
							fuckingDumbassBullshitFuckYou = 'LEFT';
	...
	function swapGlitch(glitchTime:Float, toBackground:String) {
		if (FlxG.save.data.stage && !FlxG.save.data.lowQ) {
			...
			var glitch = new BGSprite('glitch', 0, 0, 'ui/glitch/glitchSwitch', [new Animation('glitch', 'glitchScreen', 24, true, [false, false])], 0, 0,
				false, true);
	...
	override function stepHit() {
		...
		switch (SONG.song.toLowerCase()) {
			...
			case 'corn-theft':
				switch (curStep) {
					...
					case 848:
						defaultCamZoom -= 0.2;
					case 916:
						if (FlxG.save.data.flashing) FlxG.camera.flash();
					case 935:
						defaultCamZoom += 0.2;
						black = new FlxSprite().makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
						black.screenCenter();
			...
			case 'multidimensional':
				switch (curStep) {
					case 1024:
						bg1 = new BGSprite('void', -800, -500, 'backgrounds/void/multidimentional/Trippy', null, 1, 1, false, true);
						sprites.add(bg1);
						voidShader(bg1);
						bg1.setGraphicSize(Std.int(bg1.width * 1.3));
						bg1.updateHitbox();
						add(bg1);
					case 1536:
						bg1 = new BGSprite('void', -800, -500, 'backgrounds/void/multidimentional/PIlls', null, 1, 1, false, true);
						sprites.add(bg1);
						voidShader(bg1);
						bg1.setGraphicSize(Std.int(bg1.width * 1.3));
						bg1.updateHitbox();
						add(bg1);
					case 2048:
						bg1 = new BGSprite('void', -800, -500, 'backgrounds/void/multidimentional/Tubed', null, 1, 1, false, true);
						sprites.add(bg1);
						voidShader(bg1);
						bg1.setGraphicSize(Std.int(bg1.width * 1.3));
						bg1.updateHitbox();
						add(bg1);
					case 2560:
						bg1 = new BGSprite('void', -800, -500, 'backgrounds/void/multidimentional/DarkBlue', null, 1, 1, false, true);
						sprites.add(bg1);
						voidShader(bg1);
						bg1.setGraphicSize(Std.int(bg1.width * 1.3));
						bg1.updateHitbox();
					...
					case 2943:
						...
						add(bg1);
					case 3224:
						var bg:BGSprite = new BGSprite('bg', -600, -200, Paths.image('backgrounds/shared/sky', 'shared'), null, 0.6, 0.6);
						sprites.add(bg);
						add(bg);

						var flatgrass:BGSprite = new BGSprite('flatgrass', 350, 75, Paths.image('backgrounds/farm/gm_flatgrass', 'shared'), null, 0.65, 0.65);
						flatgrass.setGraphicSize(Std.int(flatgrass.width * 0.34));
						flatgrass.updateHitbox();
						sprites.add(flatgrass);

						var hills:BGSprite = new BGSprite('hills', -173, 100, Paths.image('backgrounds/farm/orangey hills', 'shared'), null, 0.65, 0.65);
						sprites.add(hills);
			...
			if (boyfriend.animation.finished) {
				switch (SONG.song.toLowerCase()) {
					...
					default:
						...
						if (darkLevels.contains(curStage)
							&& (SONG.song.toLowerCase() != "polygonized" || (SONG.song.toLowerCase() == "polygonized" && curBeat >= 608))
							&& formoverride != 'tristan-golden-glowing'
							&& bfTween == null) {
							boyfriend.color = nightColor;
						} else if (sunsetLevels.contains(curStage) && bfTween == null) {
